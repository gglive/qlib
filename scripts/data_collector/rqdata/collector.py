...
from rqdatac import *
RQDATA_LICENSE = ''
RQDATA_IP_PORT = ('rqdatad-pro.ricequant.com', 16011)
init('license',RQDATA_LICENSE,RQDATA_IP_PORT)


def symbol2oid(x:str):
    if x.lower().endswith('.ss'):
        return x.replace('.ss','.XSHG')
    elif x.lower().endswith('.se'):
        return x.replace('.se', 'XSHE')
    else:
        raise ValueError(x)
...
class RqdataCollector(BaseCollector):
    ...
    def get_data_from_remote(symbol, interval, start, end, show_1min_logging: bool = False):
        ...
        def _show_logging_func():
            if interval == RqdataCollector.INTERVAL_1min and show_1min_logging:
        ...
        try:
            ...
            # oid = symbol2oid(symbol)
            
            _resp = get_price(symbol, start_date=start, end_date=end, frequency=interval, adjust_type='none').loc[symbol]
            # method 1
            # ori_close =  get_price(symbol, start_date=start, end_date=end, fields='close', frequency=interval, adjust_type='none').loc[symbol]
            # _resp['factor'] = _resp['close'] / ori_close
            
            # 构造adj factor, 有点问题，reindex，
            _factor = get_ex_factor(symbol, '1900-01-01', end)
            if _factor is not None:
                # f = _factor['ex_factor'].reindex(_resp.index, fill_value=1).values#).cumprod()
                # pre_f = np.flip(np.flip(1/f).cumprod())
                # _resp['post_factor'] = np.cumprod(f)
                # _resp['pre_factor'] = np.flip(np.flip(1/f).cumprod())
                # _resp['adjclose'] = _resp['close']* _resp['pre_factor']

                _resp['factor'] =  _factor['ex_cum_factor'].reindex(_resp.index, method='ffill')  # factor = adjusted_price / original_price
            else:
                _resp['factor'] = 1

            # _resp['prev_close'] = _resp['prev_close'] * _resp['factor']
            ...
            logger.warning(
                ...
                + "Your data request fails. This may be caused by your firewall (e.g. GFW). Please switch your network if you want to access Rqdata! data"
            )
...
class RqdataCollectorCN(RqdataCollector, ABC):
    def get_instrument_list(self):
        ...
        symbols = all_instruments(type=['STOCK'], market='cn')['order_book_id'].tolist()
        # symbols = ['600000.XSHG']
    ...
    def normalize_symbol(self, symbol):
        ...
        symbol = {
            "XSHG" : f"sh{symbol_s[0]}",
            "XSHE" : f"sz{symbol_s[0]}",
        }[symbol_s[-1]]
        return symbol
...
class RqdataCollectorCN1d(RqdataCollectorCN):
    def download_index_data(self):
        ...
        for _index_name, _index_code in {"csi300": "000300.XSHG", "csi100": "000903.XSHG", "csi500": "000905.XSHG", "csi800":"000852.XSHG"}.items():
            logger.info(f"get bench data: {_index_name}({_index_code})......")
            try:
                df = get_price(_index_code, _begin, _end, frequency='1d').loc[_index_code].reset_index()
            except Exception as e:
            ...
            df["factor"] = 1
            df["symbol"] = f"sh{_index_code[:-5]}"
            _path = self.save_dir.joinpath(f"sh{_index_code[:-5]}.csv")
            if _path.exists():
...
class RqdataCollectorCN1min(RqdataCollectorCN):
    def get_instrument_list(self):
        ...
        return symbols + ["000300.XSHG", "000905.XSHG", "000903.XSHG", "000852.XSHG"]
...
class RqdataNormalize(BaseNormalize):
    COLUMNS = ["prev_close", "open", "close", "high", "low", "volume"]
    DAILY_FORMAT = "%Y-%m-%d"
...
class RqdataNormalize1d(RqdataNormalize, ABC):
    ...
    def _get_first_close(self, df: pd.DataFrame) -> float:
        ...
        Notes
        -----
            For incremental updates(append) to Rqdata 1D data, user need to use a close that is not 0 on the first trading day of the existing data
        """
    ...
    def _manual_adj_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for _col in df.columns:
            ...
            if _col in [self._symbol_field_name, "adjclose", "change", "num_trades","total_turnover"]:
                continue
            if _col == "volume":
                df[_col] = df[_col] * _close
            elif _col in ("limit_up", "limit_down"):
                df[_col] = df[_col] * df['factor'] / _close
            else:
                df[_col] = df[_col] / _close
        return df.reset_index()
...
class RqdataNormalize1dExtend(RqdataNormalize1d):
    ...
    def _get_first_close(self, df: pd.DataFrame) -> float:
        try:
            ...
            _close = super(RqdataNormalize1dExtend, self)._get_first_close(df)
        return _close
...
class RqdataNormalize1min(RqdataNormalize, ABC):
    ...
    def get_1d_data(self, symbol: str, start: str, end: str) -> pd.DataFrame:
        ...
        """
        data_1d = RqdataCollector.get_data_from_remote(self.symbol_to_Rqdata(symbol), interval="1d", start=start, end=end)
        if not (data_1d is None or data_1d.empty):
            _class_name = self.__class__.__name__.replace("min", "d")
            _class: type(RqdataNormalize) = getattr(importlib.import_module("collector"), _class_name)
    ...
    @abc.abstractmethod
    def symbol_to_Rqdata(self, symbol):
        raise NotImplementedError("rewrite symbol_to_Rqdata")
...
class RqdataNormalize1minOffline(RqdataNormalize1min):
    ...
    def __init__(
        ...
        qlib_data_1d_dir: str, Path
            the qlib data to be updated for Rqdata, usually from: Normalised to 1min using local 1d data
        date_field_name: str
            date field name, default is date
        symbol_field_name: str
            symbol field name, default is symbol
        ...
        super(RqdataNormalize1minOffline, self).__init__(date_field_name, symbol_field_name)
...
class RqdataNormalizeCN:
    def _get_calendar_list(self) -> Iterable[pd.Timestamp]:
        ...
        return get_trading_dates(start_date='2000-01-01', end_date=pd.Timestamp.now() + pd.Timedelta(days=31))
...
class RqdataNormalizeCN1min(RqdataNormalizeCN, RqdataNormalize1minOffline):
...
class Run(BaseRun):
    ...
    def normalize_data_1d_extend(
        ...
        """normalize data extend; extending Rqdata qlib data(from: https://github.com/microsoft/qlib/tree/main/scripts#download-cn-data)

        Notes
        -----
            Steps to extend Rqdata qlib data:

                1. download qlib data: https://github.com/microsoft/qlib/tree/main/scripts#download-cn-data; save to <dir1>

                2. collector source data: https://github.com/microsoft/qlib/tree/main/scripts/data_collector/Rqdata#collector-data; save to <dir2>

                3. normalize new source data(from step 2): python scripts/data_collector/Rqdata/collector.py normalize_data_1d_extend --old_qlib_dir <dir1> --source_dir <dir2> --normalize_dir <dir3> --region CN --interval 1d
